...
if(isset($_POST["btnsave"]))
{
	...
	if($password == $repassword)
	{
		...
		$checkWorkshop = "SELECT * FROM tbl_workshop WHERE workshop_email = '$email'";
		$result = $conn->query($checkWorkshop);

		if($result->num_rows > 0)  // Proper check
		{
			?>
			<script>
		...
		{
			$insqry = "INSERT INTO tbl_workshop (workshop_name, workshop_contact, workshop_email, workshop_address, workshop_password, workshop_logo, workshop_proof, workshop_doj, workshop_location, workshoptype_id)
					   VALUES ('$name', '$contact', '$email', '$address', '$password', '$logo', '$proof', CURDATE(), '$location', '$type')";
...
<tr>
    <td>Name</td>
    <td>
      <input type="text" name="txtname" id="txtname" required autocomplete="off"/>
    </td>
</tr>
...
<tr>
    <td><p>Contact</p></td>
    <td>
      <input type="text" name="txtcontact" id="txtcontact" required pattern="[0-9]{10,10}" autocomplete="off"/>
    </td>
</tr>
...
<tr>
    <td>Address</td>
    <td>
      <textarea name="txtadd" id="txtadd" cols="45" rows="5" required></textarea>
    </td>
</tr>
...
<tr>
    <td>Password</td>
    <td>
      <input type="password" name="txtpassword" id="txtpassword" required autocomplete="off" pattern="[0-9a-zA-Z!@#$%^&*]{8,}"/>
    </td>
</tr>
...
<tr>
    <td>Re-Password</td>
    <td>
      <input type="password" name="txtrepassword" id="txtrepassword" required autocomplete="off"/>
    </td>
</tr>
